...
switch($_POST['op']) {			//change back to POST
	...
	$query = sprintf("SELECT DISTINCT settings.hostname, settings.data AS remoteIsEnabled, settings1.data AS port
					  FROM settings JOIN settings AS settings1 ON (settings.hostname = settings1.hostname)
			WHERE ((settings.hostname IS NOT NULL) AND ((settings.value = 'NetworkControlEnabled') 
					AND (settings1.value = 'NetworkControlPort')))"
		);
      break;
    }
